...
The `MovieList` component SHALL expose its inputs as three public Razor `[Parameter]` properties on the component class: `Movies` of type `MovieListResponse?`, `IsLoading` of type `bool`, and `CategoryLabel` of type `string`. Each of these parameters MUST be annotated with `[EditorRequired]` together with `[Parameter]` (for example `[Parameter, EditorRequired]`) so that `.razor` callers receive analyzer feedback when a required attribute binding is missing. The default values MUST be `null` for `Movies`, `false` for `IsLoading`, and the empty string `""` for `CategoryLabel` so the component can be constructed without arguments in tests that disable the relevant analyzer warning.
